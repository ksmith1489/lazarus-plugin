Opening this as a fresh registration under `The Helm` after the previous `OctoGoat` submission was closed.

This resubmission addresses the concrete review items raised on `#1435`:

- the plugin has been renamed from `OctoGoat` to `The Helm` to avoid `Octo` trademark concerns;
