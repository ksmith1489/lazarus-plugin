Opening this as a fresh registration under `Failed Print Resume Wizard` after the previous `OctoGoat` submission was closed.

This resubmission addresses the concrete review items raised on `#1435`:

- the plugin has been renamed from `OctoGoat` to `Failed Print Resume Wizard` to avoid `Octo` trademark concerns;
