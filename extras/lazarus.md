...
The plugin generates the resume G-code locally inside OctoPrint from the original G-code file, the measured print height, and the detected layer structure of that file. The original G-code is not uploaded to the The Helm service for processing.

The Helm includes a guided recovery workflow:
...
Printer movement remains user-controlled. The Helm does not automatically home Z into an existing print.

Optional Moonraker/Klipper support is included through a user-provided local Moonraker address.

The Helm uses `https://wizard.lazarus3dprint.com` for activation and subscription validation. If that service is unavailable, The Helm fails closed for resume generation and execution without causing OctoPrint itself to malfunction.

The Helm is commercial software and requires an active subscription for resume generation and execution. Pricing, activation, terms, license information, and privacy information are available here:
