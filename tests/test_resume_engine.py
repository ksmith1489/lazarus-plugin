...
class ResumeEngineRegressionTests(unittest.TestCase):
    ...
    def test_build_resumed_gcode_anchors_on_true_print_plane(self) -> None:
        ...
        self.assertEqual(result["datum_extreme_key"], "x_min")
        self.assertEqual(set(result["resume_layer_extremes"]), {"x_min", "x_max", "y_min", "y_max"})
        self.assertTrue(result["resume_layer_extremes"]["x_min"]["is_datum"])
        self.assertAlmostEqual(result["resume_layer_extremes"]["x_min"]["x"], 157.948, places=3)
        self.assertAlmostEqual(result["resume_layer_extremes"]["x_min"]["z"], 26.32, places=3)
